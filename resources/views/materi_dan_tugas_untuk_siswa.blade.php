...
<body>
  ...
  <!-- tugas kelas kita -->
  <div class="container">
      <h2 class="mb-4 text-center mb-3 mt-5 main-heading">Kerjakan Tugas</h2>

      @foreach ($tugaskelas as $tugas)
       @if (\Carbon\Carbon::parse($tugas->deadline)->isFuture())
      <div class="card mb-5">
          <div class="card-header">
              <h4>{{ $loop->iteration }} . {{ $tugas->title }}</h4>
          </div>
          <div class="card-body">
              <p><strong>Deskripsi:</strong> {{ $tugas->description }}</p>
              <p><strong>Deadline:</strong> {{ \Carbon\Carbon::parse($tugas->deadline)->format('d M Y H:i') }}</p>
              <p><strong>File Tugas:</strong></p>
              <a href="{{ asset($tugas->file_url) }}" target="_blank" class="btn btn-sm btn-primary">
                  Download File
              </a>

              <hr>
              <div class="card-header bg-primary text-white">
                  <h4>Kumpulkan tugas & jawaban</h4>
              </div>
              <!-- Form Upload Jawaban -->
              <form action="{{route('kirim.tugas.siswa')}}" method="POST" enctype="multipart/form-data">
                  @csrf
                  <div class="mb-3">
                      <!-- user_id -->
                      ...
                      <label for="file_jawaban" class="form-label">Upload Jawaban (PDF/JPG/PNG)</label>
                      <input type="file" name="file_jawaban" class="form-control" accept=".pdf, image/*, video/*" required>
                  </div>
