...
<body>
  ...
  <div class="container">
    <h2 class="text-center mb-3 mt-5 main-heading">Daftar Kelas SMK</h2>
    <div class="row row-cols-1 row-cols-md-3 g-4">
      @foreach($kelas as $k)
      <div class="col">
        <a href="{{ route('show_data_siswa_tiap_kelas',$k->id) }}" style="text-decoration: none; color: inherit;">
          <div class="card shadow-sm h-100 cursor-pointer">
            <div class="card-body">
              <h5 class="card-title">RPL {{ $k->jurusan }}</h5>
              <p class="card-text">{{ $k->description }} - {{ $k->nama_kelas }}</p>
            </div>
          </div>
        </a>
      </div>
  ...
  <div class="d-flex flex-row">
    <a href="{{route ('formTambahkelas')}}" class="btn btn-primary mx-5 my-3">Tambah Kelas</a>
    <div style="flex:1;"></div>
    <a href="{{route ('listkelas')}}" class="btn btn-success mx-5 my-3">Edit Kelas</a>
  </div>
  <hr>
